...
- [p2p] [\#3369](https://github.com/zlyzol/tendermint-0.32.3/issues/3369) Do not panic when filter times out
...
- [dep] [\#2844](https://github.com/zlyzol/tendermint-0.32.3/issues/2844) Dependencies are no longer pinned to an exact version in the
  Gopkg.toml:
...
* Go API
  ...
  * [types] [\#2636](https://github.com/zlyzol/tendermint-0.32.3/issues/2636) Rename fields in ConsensusParams to remove `Params` suffixes
...
- [rpc]: changed the output format for the `/status` endpoint (see https://godoc.org/github.com/zlyzol/tendermint-0.32.3/rpc/core#Status)
...
IMPROVEMENTS:
- rpc/client: can act as event bus subscriber (See https://github.com/zlyzol/tendermint-0.32.3/issues/945).
...
- Config and Flags:
  - The `config` map is replaced with a [`Config` struct](https://github.com/zlyzol/tendermint-0.32.3/blob/master/config/config.go#L11),
...
- Merge `tendermint/go-p2p -> zlyzol/tendermint-0.32.3/p2p` and `tendermint/go-rpc -> zlyzol/tendermint-0.32.3/rpc/lib`
...
- Docker image improved, slimmed down (using Alpine), and changed from tendermint/tmbase to zlyzol/tendermint-0.32.3
